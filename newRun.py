...
from DQNConv1d import DqnConv1d
from DQN.Prioritized_Replay_DQN import DQNPrioritizedReplay
from Environments.NewEnvironment import newEnvironment
n_train=3000
...
class RunDQN:
    def __init__(self,filename):
        ...
        self.env=newEnvironment(self.filename)
...
#            RL.plot_cost()
        ...
        experimentDataFile=pd.read_excel('./experimentData.xlsx')
        Df_expData=pd.DataFrame(experimentDataFile)
        Df_expData=Df_expData.append([{'系统':self.filename,'类个数':self.class_n,'算法':method,'时间':b-start,
        ...
        print(Df_expData)
        Df_expData.to_excel('./experimentData.xlsx',sheet_name=self.filename)
...
if __name__ == '__main__':
    filenames=['ant','ATM','BCEL','daisy','DNS','elevator','Email-spl','JaConTeBe']
    file='JaConTeBe'
