...
import time
import math
from Environments.GsEnvironment import GsEnvironment
...
class RunDQN:
    def __init__(self,filename):
        ...
        self.env=GsEnvironment(self.filename)
    ...
    def runDQN(self):
        ...
        for episode in range(n_train):
            ...
            while True:
                ...
                if done:
                    ...
                    GSnum = self.env.getNumOfGS()
                    if GSnum<=self.minGStubs:
                        self.mincost=cost
                        self.minGStubs=GSnum
            ...
            if self.mincost<0.8:
                break
            print('第{}轮训练，整体测试桩复杂度为{}，测试桩个数为{}。'.format(episode,cost,GSnum))
        f=open('./experiment','a')
        ...
        # writer.save()
        experimentDataFile = pd.read_excel('.\DataOfExperiment\experimentWithGS.xlsx')
        Df_expData = pd.DataFrame(experimentDataFile)
        Df_expData=Df_expData.append([{'系统名':self.filename,'类个数':self.class_n,'算法':method,'时间':b-start,
                                    '属性复杂度':self.minAttrdeps,'方法复杂度':self.minMethoddeps,
                                    '训练轮数':n_train,'最佳测试序列':self.bestOrder,'整体测试桩复杂度':self.mincost,
                                    '通用测试桩个数':numOfGS,'通用测试桩序列':GS}])
        Df_expData.to_excel('.\DataOfExperiment\experimentWithGS.xlsx')
...
if __name__ == '__main__':
    ...
    filename2=['ant','ATM','BCEL','daisy','SPM','simple','test']
    for i in range(1):
